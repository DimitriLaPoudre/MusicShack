...
func (r *SQLiteRepository) CreateSession(ctx context.Context, s model.Session) (model.Session, error) {
	...
	if err != nil {
		return model.Session{}, fmt.Errorf("failed to create session: %w", dto.Error(err))
	}
...
func (r *SQLiteRepository) GetSessionByFilter(ctx context.Context, filter model.SessionFilter) (model.Session, error) {
	...
	if err != nil {
		return model.Session{}, fmt.Errorf("failed to get session with filter %v: %w", filter, dto.Error(err))
	}
...
func (r *SQLiteRepository) ListSessionsByFilter(ctx context.Context, filter model.SessionFilter) ([]model.Session, error) {
	...
	if err != nil {
		return []model.Session{}, fmt.Errorf("failed to list session with filter %v: %w", filter, dto.Error(err))
	}
...
func (r *SQLiteRepository) DeleteSession(ctx context.Context, sessionID uuid.UUID) error {
	...
	if err != nil {
		return fmt.Errorf("failed to delete session %s: %w", sessionID.String(), dto.Error(err))
	}
	return nil
}
...
func (r *SQLiteRepository) DeleteSessionByToken(ctx context.Context, token string) error {
	...
	if err != nil {
		return fmt.Errorf("failed to delete session via token %s: %w", token, dto.Error(err))
	}
	return nil
}
...
func (r *SQLiteRepository) DeleteSessionExpired(ctx context.Context) error {
	...
	if err != nil {
		return fmt.Errorf("failed to delete expired sessions: %w", dto.Error(err))
	}
	return nil
}
